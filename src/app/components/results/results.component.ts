      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 mb-8">
          ...
          <div class="bg-indigo-50 rounded-lg p-6">
            <h3 class="text-xl font-semibold text-indigo-900 mb-2">{{ results?.focusArea?.name ?? 'Loading...' }}</h3>
            <p class="text-gray-700">{{ results?.focusArea?.description ?? 'Loading...' }}</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <ng-container *ngFor="let round of rounds">
              <div class="bg-gray-50 rounded-lg p-6" *ngIf="results?.rounds as roundsData">
                <h3 class="text-xl font-semibold text-gray-900 mb-4">Round {{ round.slice(-1) }}</h3>
                <div class="text-3xl font-bold text-indigo-600 mb-4">{{ roundsData[round]?.score ?? 0 }}%</div>
                <div class="space-y-4">
                  <div>
                    <h4 class="font-medium text-gray-900 mb-2">Strengths</h4>
                    <ul class="list-disc list-inside text-gray-600">
                      <li *ngFor="let strength of roundsData[round]?.strengths ?? []">{{ strength }}</li>
                    </ul>
                  </div>
                  <div>
                    <h4 class="font-medium text-gray-900 mb-2">Areas for Improvement</h4>
                    <ul class="list-disc list-inside text-gray-600">
                      <li *ngFor="let area of roundsData[round]?.areas ?? []">{{ area }}</li>
                    </ul>
                  </div>
                  <div class="border-t pt-4">
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      <span class="text-lg font-semibold text-indigo-600">{{ roundsData[round]?.comparison?.humanScore ?? 0 }}%</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-gray-600">AI</span>
                      <span class="text-lg font-semibold text-gray-600">{{ roundsData[round]?.comparison?.aiScore ?? 0 }}%</span>
                    </div>
                    <div class="text-sm text-gray-500 mt-2">
                      Difference: {{ (roundsData[round]?.comparison?.difference ?? 0) > 0 ? '+' : '' }}{{ roundsData[round]?.comparison?.difference ?? 0 }}%
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div *ngFor="let badge of results?.badges ?? []" class="bg-gray-50 rounded-lg p-6 text-center">
              ...
              <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ badge.name }}</h3>
        ...
        <div class="flex justify-center space-x-4">
          ...
          <button routerLink="/" class="bg-gray-200 text-gray-800 px-6 py-3 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
            Start New Challenge
          </button>
        </div>
      </div>
